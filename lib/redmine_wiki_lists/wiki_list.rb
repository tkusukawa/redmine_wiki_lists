      ...
      begin
        ...
        args.each do |arg|
          ...
          if arg=~/\A\-([^\=]*)(\=.*)?\z/ # オプション表記発見
            ...
            if arg=~/\A(.*)\|(.*)\|(.*)\z/ then # 抽出キーワードと別にカラム表示名と列幅の指定がある場合
              column_keys.push($1.strip)
              column_names.push($2.strip+'|'+$3.strip)
            elsif arg=~/\A(.*)\|(.*)\z/ then # 抽出キーワードと別にカラム表示名の指定がある場合
              column_keys.push($1.strip)
          ...
          case column_key
            ...
            when '+project' # Wikiページのプロジェクト名
              ...
              matches.each do |match| # 抽出されたキーワード毎の処理
                # キーワードの後ろの文字列を抽出
                if terminator
                  match =~/\A#{keyword}([\s\S]*)#{terminator}/
                else
                  match =~/\A#{keyword}(.*)\z/
                end
              ...
              if new_lines.length==0 # キーワードが１つも抽出されていなかったら空文字を入れておく
